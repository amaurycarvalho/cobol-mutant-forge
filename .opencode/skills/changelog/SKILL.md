...
2.1. **Read `CHANGELOG.md`** and **`CHANGELOG-ARCHIVE.md`** in full.
   - If either file does not exist, treat it as empty — it will be created fresh in steps 7/8.
...
4.1. **Identify the current release version** from CHANGELOG.md (the first non-Unreleased `## [<version>] -` heading).
   - If CHANGELOG.md has no release entry yet, use `VERSION` from the `Makefile` (the single source of truth for the release version) as the candidate current version.
...
7.4. **Rebuild the full CHANGELOG.md**:
   ...
   - Derive the GitHub repo URL from existing links in CHANGELOG.md (preserve the existing format), or from `git remote get-url origin`, or ask the user.
...
If the version was updated in step 4.2, suggest the user run the `release-version` skill manually to update the version number in the `Makefile` (`VERSION ?=`).
